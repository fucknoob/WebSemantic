...
def init_cenario_class_cliente(stack):
  ...
  if not fnd:
   ...
   cen.rcts=['ask-aswer-aderency-product-response' ]
    ...
    for l in s:
     if type(l).__name__ == 'instance':
      if l.__class__.__name__ == 'mdLayer':
         ...
         print 'Coll.fact:',linksl
         for ln2 in linksl:
          if ln2.lr.name.upper() != 'process_aderencia_filter'.upper(): continue
          for l2 in ln2.lr.get_links('FACT') :
           ln=l2
