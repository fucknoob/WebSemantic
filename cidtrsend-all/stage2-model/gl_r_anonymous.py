...
def do_reg2(item):
    ...
    for l in links :
        ...
        for l2 in links2 :
            lnk = l2.replace('\"', '' )
            lnk=lnk.replace('http://anonymouse.org/cgi-bin/anon-www.cgi/','')
            if lnk != '#' and lnk[:4] == 'http' :
             text1=item.replace(l2, '')
             text2=text1.replace('</a>', '')
             text2=text1.replace(' \'\">','')
             
             #<a href= onmousedown="return hlprwt(this, 'http://carroonline.terra.com.br/')    
             #("<h3 class=\"r(.*?)</h3>")             
             ks= re.compile('<a href= onmousedown=\"(.*?)\)')
             ksl = ks.findall(text2)       
             
             for l2 in ksl :
               l2='<a href= onmousedown=\"'+l2+')'
               text2=text2.replace(l2,'')
...
def open_pg(endereco):   
 proc = subprocess.Popen("php /home/mindnet/public_html/get_Text.php q="+endereco, shell=True,stdout=subprocess.PIPE)
...
def run(qr,icount):
 ...
 while r1 < reps:
  istart=(r1*100)+1
  #print 'Search....'
  url='http://anonymouse.org/cgi-bin/anon-www.cgi/http://www.google.com/search?hl=pt&num='+str(icount)+'&start='+str(istart)+'&safe=off&q='+qr
...
c= run('carro',10) 
f=open('/home/mdnetsearc/kk.txt','w');
for c1 in c:
 for c2 in c1:
  f.write(c2)
  f.write('<--->')
