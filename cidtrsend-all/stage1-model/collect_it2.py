...
def search_public(query):
    arqs={}
    arqs["q"], arqs["rpp"] ,arqs["show_user"] = query, "100","true"
    url='http://search.twitter.com/search.json?'+urllib.urlencode(arqs) 
...
def get_public(query):   
  ...
  for r in res:
    id_msg=r['id_str']
    id_usr=r['from_user_id_str']
    usr=r['from_user']
    dt=r['created_at']
    msg=r['text']
    to_usr=r['to_user_id']
    msg_picture=r['profile_image_url']
    source=r['source']
    #==
...
def post_termo(it,termo,purpose,user):
 ...
 try:
  conn.sqlX(" insert into web_cache(url,pg,termo,usr,purpose,processed,url_icon,url_picture,id_usr,name_usr,story,title,doc_id,tp,tps) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,'T') ",mt) 
 except Exception,e:
  print 'Error post twitter(1):',e,mt
 conn.commit() 
 
 
def processa_termo( query,usr,purpose ): 
  print 'Process post twitter...'
...
def clean(USERNAME,termo,purpose):
 mt=[USERNAME,termo]
 conn.sqlX(" delete from web_cache where usr=? and  termo=? and tps='T'  ", mt)
 conn.commit () 
...
def run_th(user,termo,purp,th,pth):
    cmd='python '+pth+'collect_it2.py '+ '"' + user +'\" \"'+ termo + ' \" \"' + purp + '"  '
    os.system(cmd)
    th.finished=True  
...
def hava_not_indexed(USER,TERMO):
 sqlcc='select * from web_cache where USR=? and termo= ? and indexed = \'N\' and  tps=\'T\' and rowno < 2 ' 
