...
def init_nums_i():
  ...
  cache_i2=[]
  try:
     sq="select i from web_cache3 where USR='"+usr2+"' and (indexed='N') and rowno <= 2000 order by i"
     cursor = conn.sql (sq)
...
def get_feeds( space1  ):
 if True:
    try: 
     ...
     cmd='python32 entry_SemaIndexerStage1.py \"igor.moraes\" \"coll-usr-need\" \"'+space1+'"\ '
     os.system(cmd)
     #print cmd,'\n'
    except: pass 
