...
a_lock1 = thread.allocate_lock()
a_lock2= thread.allocate_lock()
...
def get_feeds(id,cs,c_url,th,to_ins,pg_id,user):
 try:
    ...
    try:
     ...
     get_feeds(id,(cs+1),nova_pagina,th,[],pg_id,user)
    except: pass
...
def get_feeds2(id,cs,c_url,th,to_ins,pg_id,user):
 try:
    ...
    try:
     pagin=result['paging']    
     nova_pagina=pagin['next']     
     get_feeds2(id,(cs+1),nova_pagina,th,[],pg_id,user)
    except: pass
...
def get_feeds3(id,cs,c_url,th,to_ins,pg_id,user):
 try:
    ...
    try:
     pagin=result['paging']    
     nova_pagina=pagin['next']     
     get_feeds3(id,(cs+1),nova_pagina,th,[],pg_id,user)
    except: pass
